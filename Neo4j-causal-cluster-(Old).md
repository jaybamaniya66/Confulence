 **Configuration** : Installation and configure steps for Neo4j causal cluster on AWS ec2-instance.

Step 1: Installation Step to neo4j enterprise version 4.2.3
...
sudo apt-get update
sudo apt-get install neo4j-enterprise=1:4.2.7
neo4j-admin memrec
...
Step 2: Change limits of the ubuntu server: cat /etc/security/limits.conf
...
```
Step 2.1: Note: You Need to Reboot that server after Step 2.

reboot



Step 3: Always use neo4j user for further changes(note: please check you are not in the root user).
...
Step 4: Add Additional librabry.

 Apoc library souce (check the version for the same & Download the “all.jar” file) :  [https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags](https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags)

For Example (If I want to version 4.4.0.3):

 wget [https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags](https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags)



Step 5:For updating the neo4j.conf file for reference a link is given below.

[https://github.com/upvision-in/devops-assets/blob/master/orgs/FulcrumSaaS/Neo4j/Neo4j_causal_clustering_neo4j.conf](https://github.com/upvision-in/devops-assets/blob/master/orgs/FulcrumSaaS/Neo4j/Neo4j_causal_clustering_neo4j.conf)


```
dbms.default_listen_address=0.0.0.0

dbms.default_advertised_address=(Public ip of instance/droplet)

#dbms.connector.bolt.listen_address=0.0.0.0:7687
#dbms.connector.bolt.advertised_address=(Public ip of instance/droplet):7687

#dbms.connector.http.listen_address=0.0.0.0:7474
#dbms.connector.http.advertised_address=(Public ip of instance/droplet):7474

dbms.connector.https.listen_address=0.0.0.0:7473
dbms.connector.https.advertised_address=(Public ip of instance/droplet):7473

dbms.mode=CORE

causal_clustering.minimum_core_cluster_size_at_runtime=2
causal_clustering.initial_discovery_members=(Public_ip-1):5000,(Public_ip-2):5000,(Public_ip-3):5000
```


Step 5: Check for all TCP ports are the open inbound rule. 

Neo4j uses (5000, 6000, 7000, 7474, 7687) Ports.

 _Update Firewall for neo4j cluster config as above ports._ 

Step 6: Add DNS in google domain for the particular server.

Step 7: Restart and check the status of all neo4j cluster instances/droplets one by one.

service neo4j restart

service neo4j status

Step 7: Add DNS in google domain for the particular server.

For Example: neo4j.conf
...
dbms.default_advertised_address=137.184.118.3
...
dbms.connector.bolt.advertised_address=hyp-neo-v2-02-04.fulcrumsaas.net:7687
...
dbms.connector.https.listen_address=0.0.0.0:7473
dbms.connector.https.advertised_address=137.184.118.3:7473
...
causal_clustering.initial_discovery_members=157.245.13.187:5000,157.245.3.173:5000,157.245.3.73:5000
...
metrics.prometheus.endpoint=10.124.0.5:2004
...
Step 5: Open neo4j browser on public_ip:7474. to check more in who is leader and follower.

:sysinfo



Step 6: Testing Basic Cypher Query.
...
SHOW DATABASES;
use movie;
#Copy from Neo4j Browser Play movies data into cypher-shell
CREATE (TheMatrix:Movie {title:'The Matrix’,… … … … … … (d) RETURN a,m,d LIMIT 10;
...
Step 7: Create New User From the neo4j browser.
...
CREATE USER vedant SET PASSWORD ‘Vedant@123' CHANGE REQUIRED SET STATUS SUSPENDED;
...
Role: admin
```
Step 8: Login and Test new user in Cypher-shell


```
/usr/bin/cypher-shell -u vedant -p vedant -a bolt://54.166.181.89:7687 -d movies
show databases;
show users;

Test:MATCH (p:Person) WHERE p.name='Tom Cruise' RETURN p.name, p.born;
```


Step 9: Seeding of Cluster.
...
```
For Uninstall neo4J, Below are the commands that can be executed :
