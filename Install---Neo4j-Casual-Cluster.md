...
1. Install Node Exporter : [https://upvision-in.atlassian.net/l/c/72LNrFVT](https://upvision-in.atlassian.net/l/c/72LNrFVT)


```
sudo su

# https://github.com/upvision-in/devops-assets/raw/master/scripts/shell/install/install_node_exporter.sh
# Change the raw file Url below : Browse to the raw file and then paste the Url here for it to work in the SSH terminal window
wget https://raw.githubusercontent.com/upvision-in/devops-assets/master/scripts/shell/install/install_node_exporter.sh?token=GHSAT0AAAAAABR2XLKZNN6HZQLMRIIJZ7PCYTBCCWA -O install_node_exporter.sh

chmod 744 install_node_exporter.sh
./install_node_exporter.sh
service node_exporter status
netstat -tunlp

rm install_node_exporter.sh
```
...
# sample settings - change as per your needs
* soft nofile 60000
* hard nofile 60000
...
1. Install Neo4j - This will install the latest stable release version of neo4j
...
1. Perform all future steps using “ **neo4j** ” user account
...
1. Install APOC:  (Link for the apoc libraray for reference: [https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags](https://github.com/neo4j-contrib/neo4j-apoc-procedures/tags)  )


```
# Make sure to check the lab folder for confirming the version of the apoc library. If warning comes try upgrading or degarding the version of apoc until the warning doesn't show up
wget -P /var/lib/neo4j/plugins/ https://github.com/neo4j-contrib/neo4j-apoc-procedures/releases/download/4.4.0.5/apoc-4.4.0.5-all.jar
chown neo4j:adm /var/lib/neo4j/plugins/apoc-4.4.0.5-all.jar
# Make directory in /var/run:
mkdir /var/run/neo4j 
# Change the ownership to neo4j:
chown -R neo4j:neo4j /var/run/neo4j
```
...
```
mkdir C:/Temp/fulcrum-certs
scp -i .ssh/fulcrum-do-20201214 -r root@164.90.136.99:/var/lib/neo4j/certificates /mnt/c/temp/fulcrum-certs/
scp -i .ssh/fulcrum-do-20201214 -r /mnt/c/temp/fulcrum-certs/certificates root@165.232.142.210:/var/lib/neo4j/
sudo chown -R neo4j:adm /var/lib/neo4j/certificates/
```



1. Remove the source list file of neo4j from the following folder: (Note: this will remove the debian file for neo4j which will not upgrade or upgrading when we do it for the whole OS)
...
1. Configure Neo4j : Following information is added/updated in the neo4j.conf file


```
dbms.default_listen_address=0.0.0.0

dbms.default_advertised_address=157.245.214.232

dbms.connector.bolt.tls_level=REQUIRED
dbms.connector.bolt.listen_address=:7687
dbms.connector.bolt.advertised_address=hyp-neo-v3-s01-01.fulcrumsaas.net:7687

dbms.connector.https.enabled=true
dbms.connector.https.listen_address=:7473
dbms.connector.https.advertised_address=157.245.214.232:7473
...
causal_clustering.initial_discovery_members=157.245.214.232:5000,167.172.226.115:5000,167.172.225.102:5000
...
dbms.memory.pagecache.size=81100m
...
# The IP (of neo4j install server) and port the endpoint will bind to in the format <hostname(hyp-v2-neo-public-ip) or IP address>:<port number>.
metrics.prometheus.endpoint=157.245.214.232:2004
...
dbms.connector.bolt.thread_pool_keep_alive=1m
dbms.connector.bolt.thread_pool_min_size=200
...
1. Add DNS in google domain for the particular server.

    Note : For single instance / for the first instance of the cluster - Open neo4j browser on (IP address or DNS if created) 164.90.136.99:7474 ( Note : For the first time you logged in, default User/Password : neo4j / neo4j )

    


1. Restart neo4j services
...
https://neo-v3-testing-01.fulcrumsaas.net:7473/browser/
# The default credentials are neo4j / neo4j

# Test APOC on the neo4j browser UI
RETURN apoc.version()
```



1.  Create New User From the neo4j browser. 
...
Add User In Neo4j Browser as bellow
:server user add

Add the name & password and give the role to the user as per need (below is demo from one of the server):
Name:sysops
Pass: fruit-blitz-angel
Confirm: fruit-blitz-angel
Role: admin /public
```



1.  Login and Test new user in Cypher-shell
...
```
sudo apt-get remove --purge neo4j-enterprise
```


Seeding of the neo4j Cluster: 

Docs for Seed: [https://neo4j.com/docs/operations-manual/current/clustering/seed/#causal-clustering-seed-from-backups](https://neo4j.com/docs/operations-manual/current/clustering/seed/#causal-clustering-seed-from-backups)

Docs for Unbind: [https://neo4j.com/docs/operations-manual/current/tools/unbind/#neo4j-admin-unbind](https://neo4j.com/docs/operations-manual/current/tools/unbind/#neo4j-admin-unbind)


```
# Stop service for backup Data
service neo4j stop
mkdir /neo4j-seed
/usr/bin/neo4j-admin dump --database=neo4j --to=/neo4j-seed/neo4j.dump
# Result:->WARNING: Max 1024 open files allowed, minimum of 40000 recommended. See the Neo4j manual.
# Done: 69 files, 1.028MiB processed.
/usr/bin/neo4j-admin dump --database=system --to=/neo4j-seed/system.dump
# Result:->WARNING: Max 1024 open files allowed, minimum of 40000 recommended. See the Neo4j manual.
# Done: 75 files, 1.299MiB processed.
/usr/bin/neo4j-admin dump --database=movies --to=/neo4j-seed/movies.dump
# Result:->WARNING: Max 1024 open files allowed, minimum of 40000 recommended. See the Neo4j manual.
# Done: 68 files, 974.3KiB processed.

chmod -R 777 /neo4j-seed


# From local machine terminal
# copy the dumps to local machine
scp -i /Users/vedantbhavsar/FinalKimai.pem -r ubuntu@3.81.37.96:/neo4j-seed/neo4j.dump  /Users/vedantbhavsar/Documents/neo4j-seed/neo4j.dump
scp -i /Users/vedantbhavsar/FinalKimai.pem -r ubuntu@3.81.37.96:/neo4j-seed/movies.dump  /Users/vedantbhavsar/Documents/neo4j-seed/movies.dump
scp -i /Users/vedantbhavsar/FinalKimai.pem -r ubuntu@3.81.37.96:/neo4j-seed/system.dump  /Users/vedantbhavsar/Documents/neo4j-seed/system.dump

# Create new ec2 instance and dump local machine data.


# Login into cypher-shell and Drop Neo4j and System DATABASE For Load data.
# From local machine terminal
# copy the dumps to the other core servers
scp -i /Users/vedantbhavsar/FinalKimai.pem /Users/vedantbhavsar/Documents/neo4j-seed/neo4j.dump -r ubuntu@35.168.21.42:/neo4j-seed/
scp -i /Users/vedantbhavsar/FinalKimai.pem /Users/vedantbhavsar/Documents/neo4j-seed/system.dump -r ubuntu@35.168.21.42:/neo4j-seed/
scp -i /Users/vedantbhavsar/FinalKimai.pem /Users/vedantbhavsar/Documents/neo4j-seed/movies.dump -r ubuntu@35.168.21.42:/neo4j-seed/
# Check in ubuntu@34.226.217.169:$ ls -la /neo4j-seed/

# Drop database neo4j and system to load data by cypher-shell.
# Remaining two server of cluster have to do neo4j-admin unbind. 

neo4j-admin load --from=/neo4j-seed/movies.dump --database=movies --force
neo4j-admin load --from=/neo4j-seed/neo4j.dump --database=neo4j --force
...
### Sample Neo4j.conf file
Click here to see the file content
```
#*****************************************************************
# Neo4j configuration
#
...
#dbms.connector.http.advertised_address=157.245.214.232:7474
...
metrics.prometheus.enabled=true
# The IP (of neo4j install server) and port the endpoint will bind to in the format <hostname(hyp-neo-v3-public-ip) or I
P address>:<port number>.
